...
description: |
  ...
  host-agent subagent reviews — no LLM endpoint needed), local (run ocr
  against main...HEAD with the ticket file as background), ci (GitLab CI
  auto-review on MR push, findings posted as inline discussions).
...
| 模式 | 触发 | ocr 在哪跑 | 结果来源 |
|---|---|---|---|
| `delegate`（默认） | 每次评审 | 本地（ocr 只做文件筛选与规则解析，不调 LLM） | 当前 agent 内新开的独立子代理评审 |
...
- 以**独立子代理**（Task，新实例、不共享实现会话上下文）执行逐文件评审：diff（按 preview 给的 merge_base）+ 规则组 + 票 DoD，输出 path/content/start_line/end_line/category/severity 结构
- 子代理模型：默认按 `agents-config.dispatch.model`（economy）取当前宿主可用范围内经济性最高的一档；评审严格性由规则组、票 DoD 与红线复核保证，不靠模型档位
...
- 报告路径与结论返回调用方（`/implement` 子代理或主会话），供回修或主会话合并判断
...
- 评审必须由独立实例执行（ci 模式天然独立——CI 环境；local / delegate 模式不得在实现会话内代评）
