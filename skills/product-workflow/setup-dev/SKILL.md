...
description: |
  ...
  review runs as a fresh subagent inside the current agent), then writes
  ./dev/agents-config.md plus CI/rule seeds. Run once before /tech-spec,
  /issue-split, /implement, /ai-review.
allowed-tools:
  - Read
  - Write
  - Edit
...
默认形态（落进配置，不逐项问）：**执行与评审都在当前 agent 内完成**——issue 生成后由主会话新开独立子代理会话派发实现（`dispatch.executor: subagent`），子代理模型默认取宿主可用范围内经济性最高的一档（`dispatch.model: economy`）；评审默认 `ocr.mode: delegate`（ocr 只筛文件 / 解析规则，独立子代理评审，无需 LLM 端点）；质量门日常只跑 fast 档，full 档保留到交付前。
...
探查结果先回显给人，作为后续推荐依据。内网 GitLab 已确认可用（2026-09-09），tracker 默认推荐 `gitlab`；评审与执行默认都在当前 agent 内完成（delegate + subagent 派发），CI 接入是可选项而非前提。
...
2. **ocr.mode**：`delegate`（**默认推荐**：评审在当前 agent 内以新开的独立子代理执行，ocr 只做文件筛选与规则解析，不需要 LLM 端点、不需要 CI）/ `local`（push 前本地跑 ocr，需端点）/ `ci`（已接 / 准备接 GitLab CI，需端点）
...
8. **max-fix-rounds**：默认 5，可调

### Step 2：写配置与种子

按模板落盘（写作前必须先读对应模板）：

- `./dev/agents-config.md`：按 `shared/templates/agents-config.md`（相对本 SKILL.md 为 `../shared/templates/agents-config.md`）；含 `dispatch` 节（subagent 派发 + economy 模型档）与质量门 fast / full 默认规则
