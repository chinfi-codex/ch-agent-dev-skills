...
> 默认形态：**执行与评审都在当前 agent 内完成**——issue 生成后由主会话新开独立子代理会话派发实现，评审走 ocr `delegate` 模式；GitLab CI 与 LLM 端点均为可选增强，不是前提。
...
# 执行派发（issue 生成后由主会话驱动，无需人工触发）
dispatch:
  executor: subagent   # subagent = 主会话在当前 agent 内新开独立子代理会话执行 /implement
  model: economy       # 子代理默认模型：当前宿主可用范围内经济性最高的一档（低主会话一档）；
                       # 仅高风险 / 复杂票显式升级
...
ocr:
  mode: delegate          # delegate（默认）= ocr 只筛文件 / 解析规则，评审由当前 agent 内新开的独立子代理执行，无需 LLM 端点
...
- `ocr.mode: delegate` 是**默认模式而非降级**：OCR 侧不调 LLM（只做文件筛选 + 规则解析），评审智能由当前 agent 内新开的独立宿主子代理承担；无 LLM 端点、未接 CI 也能获得同等纪律的独立评审
- 评审与实现的子代理模型默认取 `dispatch.model`（经济性最高档）；评审严格性由 rule.json、票 DoD 与红线复核保证，不依赖模型档位
